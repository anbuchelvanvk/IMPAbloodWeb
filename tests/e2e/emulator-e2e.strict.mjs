...
async function main() {
  ...
  try {
    ...
    const guestDonorsPage = await api('getPublicDonorsPaged', { pagination: { limit: 40 }, filters: {} }, null);
    const guestDonors = guestDonorsPage?.items || [];
    assert(Array.isArray(guestDonors), 'Guest public donors should return array');
    assert(guestDonors.length >= 1, 'Guest public donors should include at least one donor');
    assert(guestDonors.every((d) => !d.contact), 'Guest public donors must not expose contact');
    assert(guestDonors.every((d) => d.contactProtected === true), 'Guest donors should be marked protected');

    const userDonorsPage = await api('getPublicDonorsPaged', { pagination: { limit: 40 }, filters: {} }, userToken);
    const userDonors = userDonorsPage?.items || [];
    assert(Array.isArray(userDonors), 'Authed public donors should return array');
    assert(userDonors.some((d) => Boolean(d.contact)), 'Authed public donors should expose contact');
    assert(userDonors.every((d) => d.contactProtected === false), 'Authed donors should not be protected');
    ...
    assert(openReq.hasProofImage === true, 'Public list should expose hasProofImage');
    assert(!('proofImage' in openReq), 'Public list should not expose proofImage payload');
    const publicDetail = await api('getPublicRequestById', { requestId: createdRequest.id }, null);
    assert(Boolean(publicDetail.proofImage), 'Public detail should include proof image');
